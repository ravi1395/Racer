...
public class DeadLetterQueueService implements RacerDeadLetterHandler {
     ...
     *
     * <p>Returns messages in FIFO order (oldest first), matching the order in which
     * {@link #dequeue()} will pop them. The underlying Redis list stores entries in
     * newest-first order (LPUSH), so the raw range result is reversed before emission.
     *
     * <p>Corrupted entries (entries that cannot be deserialized) are logged at WARN
     * and skipped rather than silently dropped.
     *
     * @since v1.6.0 — prior to this version the order was newest-first
     */
    public Flux<DeadLetterMessage> peekAll() {
        return reactiveStringRedisTemplate.opsForList()
                ...
                .collectList()
                .flatMapMany(list -> {
                    // LPUSH stores newest at head; reverse so callers see the same FIFO order
                    // as successive dequeue() calls.
                    Collections.reverse(list);
                    return Flux.fromIterable(list);
                })
                .concatMap(json -> deserializeDlm(json)
