 ...
 *
 * <p>
 * Used as a zero-overhead placeholder when Micrometer
 * ({@code spring-boot-starter-actuator})
 * is not on the classpath. Injected automatically by
 * {@code RacerAutoConfiguration} in place
 * of {@code null}, so that all Racer components can call metrics methods
 * unconditionally.
 */
public final class NoOpRacerMetrics implements RacerMetricsPort {

    /**
     * Singleton instance; use instead of {@code new NoOpRacerMetrics()} to avoid
     * allocation.
     */
    public static final NoOpRacerMetrics INSTANCE = new NoOpRacerMetrics();
    ...
    @Override
    public void registerThreadPoolGauges(ThreadPoolExecutor executor) {
    }

    @Override
    public void registerAutoConcurrencyGauge(String listenerId, Supplier<Number> concurrencySupplier) {
    }
