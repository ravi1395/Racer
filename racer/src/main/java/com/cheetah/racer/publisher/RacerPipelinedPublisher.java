...
/**
 * Batch publisher that issues multiple Pub/Sub {@code PUBLISH} commands in
 * parallel
 * rather than sequentially (R-9 — Throughput Optimisation).
 *
 * <h3>Why this is faster</h3>
 * The existing {@link RacerTransaction} uses {@link Flux#concat}, which waits
 * for each
 * command's reply before sending the next one (one round-trip per message).
 * This publisher uses {@link Flux#mergeDelayError}, which issues all commands
 * without
 * waiting — Lettuce (the reactive Redis driver) automatically pipelines
 * concurrent
...
public class RacerPipelinedPublisher {
        ...
        public RacerPipelinedPublisher(ReactiveRedisTemplate<String, String> redisTemplate,
                ...
                this(redisTemplate, objectMapper, maxBatchSize, (RacerMetricsPort) racerMetrics, null);
        }
         ...
         *         errors from individual messages are collected rather than
         *         short-circuiting
         */
        public Mono<List<Long>> publishBatch(String channelName, List<String> payloads, String sender) {
                ...
                return Flux.fromIterable(batches)
                                ...
                                .map(listOfLists -> listOfLists.stream()
                                                .flatMap(List::stream)
                                                .toList())
                                .doOnSuccess(counts -> log.debug("[racer-pipeline] Pipelined {} messages to '{}'",
                                                counts.size(), channelName));
        }

        /**
         * Publishes a batch of (possibly different) {@link PipelineItem} items in
         * parallel.
         * Items may target different channels, making this suitable for multi-channel
         * batch
                ...
                List<Mono<Long>> ops = items.stream()
                                .map(item -> {
                                        Mono<Void> validate = schemaRegistry != null
                                                        ? schemaRegistry.validateForPublishReactive(item.channelName(),
                                                                        item.payload())
                                                        : Mono.empty();
                                        return validate
                                                        .then(MessageEnvelopeBuilder.build(objectMapper,
                                                                        item.channelName(), item.sender(),
                                                                        item.payload()))
                                                        .flatMap(json -> redisTemplate
                                                                        .convertAndSend(item.channelName(), json))
                                                        .doOnSuccess(count -> racerMetrics.recordPublished(
                                                                        item.channelName(), "pubsub-pipeline"));
                                })
                                .toList();

                // mergeDelayError fires all Monos concurrently — Lettuce pipelines them
                // automatically
        ...
        private Mono<List<Long>> publishChunk(String channelName, List<String> payloads, String sender) {
                // Resolve schema once per batch — getSchema() is called exactly once regardless
                // of batch size.
                // Mono.cache() memoizes the result so all N payloads share the same schema
                // lookup.
                // When no schema is registered, schemaMono completes empty and validation is
                // skipped for
                ...
                List<Mono<Long>> ops = payloads.stream()
                                .map(payload -> {
                                        ...
                                        Mono<Void> validate = schemaRegistry != null
                                                        ...
                                                        : Mono.empty();
                                        return validate
                                                        .then(MessageEnvelopeBuilder.build(objectMapper, channelName,
                                                                        sender, payload))
                                                        .flatMap(json -> redisTemplate.convertAndSend(channelName,
                                                                        json))
                                                        .doOnSuccess(count -> racerMetrics.recordPublished(channelName,
                                                                        "pubsub-pipeline"));
                                })
                                .toList();
