 ...
 * Every {@value #TUNE_INTERVAL_SECONDS} seconds the tuner samples the processed and
 * failed counters for the listener it owns and applies the following rules:
 * <ol>
 *   <li><b>Error rate &gt; 10 %</b> — multiply workers by 0.75 (multiplicative back-off).
 *       This reacts quickly to downstream pressure.</li>
 *   <li><b>Throughput improved since last interval</b> — add one worker (additive increase),
 *       up to {@link #MAX_CONCURRENCY} ({@code 10 × availableProcessors()}).</li>
 *   <li><b>Throughput dropped</b> — subtract one worker; the current concurrency is above
 ...
 * {@code reducePermits}) is the actual gate. Each message must acquire a permit before
 * entering dispatch and releases it via {@code doFinally}, regardless of success or failure.
 * The {@code flatMap} upstream uses {@code Integer.MAX_VALUE} so the reactive pipeline
 * never back-pressures; the semaphore is the sole throttle.
 *
 * <h3>Starting concurrency</h3>
 * Begins at {@code 2 × availableProcessors()}, which is a safe midpoint for mixed
 * I/O and CPU workloads. The tuner ramps up by +1 every interval as long as throughput
 * keeps improving, converging to the workload's natural ceiling within a few minutes.
 */
@Slf4j
class AdaptiveConcurrencyTuner {
    ...
    /** Constructor used by tests — ceiling defaults to {@code 10 × availableProcessors()}. */
    ...
    /**
     * Production constructor — ceiling is driven by the configured thread-pool max size
    ...
    AdaptiveConcurrencyTuner(String listenerId,
        ...
        tuneExecutor = Executors.newSingleThreadScheduledExecutor(r -> {
            Thread t = new Thread(r, "racer-tuner-" + listenerId);
            t.setDaemon(true);
            return t;
        });
        tuneExecutor.scheduleAtFixedRate(
    ...
    /** Blocks until a worker slot is available. Must be called on a boundedElastic thread. */
    ...
    /**
     * Exposes the {@code protected} {@link Semaphore#reducePermits(int)} method so the
     * tuner can shrink the permit count without draining and re-issuing, which would
