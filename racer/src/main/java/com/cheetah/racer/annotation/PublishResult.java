...
public @interface PublishResult {
    ...
    /**
     * Per-annotation priority level override.
     *
     * <p>When set to any value other than {@link PriorityLevel#NONE} (the default), the return value
     * is published to the priority sub-channel {@code <channel>:priority:<LEVEL>} instead of the
     * base channel — regardless of whether a method-level {@link RacerPriority} annotation is present.
     *
     * <p>When {@link PriorityLevel#NONE} (the default), falls back to the method-level
     * {@link RacerPriority} annotation if present, or publishes to the standard (non-priority) channel.
     *
     * <p>This attribute is primarily useful when multiple {@code @PublishResult} annotations are
     * present on the same method and only <em>some</em> channels require priority routing.
     */
