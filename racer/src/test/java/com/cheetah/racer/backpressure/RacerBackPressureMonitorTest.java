...
class RacerBackPressureMonitorTest {
    ...
    void deactivates_notifiesBothRegistrars() {
        ...
        // Pub/Sub is released immediately on first below-threshold tick
        verify(listenerRegistrar).setBackPressureActive(false);
        // Stream recovery is graduated: first step halves backoffMs (2000 → 1000), not a snap to 0
        verify(streamListenerRegistrar).setBackPressurePollIntervalMs(1_000);
    }
