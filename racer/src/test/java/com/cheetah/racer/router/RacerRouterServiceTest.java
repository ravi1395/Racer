...
class RacerRouterServiceTest {
    ...
    @Test
    void dryRun_exactMatch_returnsForwarded() {
    ...
    @Test
    void dryRun_regexMatch_returnsForwarded() {
        // "NOTIFY.*" matches "NOTIFY_EVENT" (starts with NOTIFY, then any chars)
        RouteDecision decision = routerService.dryRun("{\"type\":\"NOTIFY_EVENT\"}", null, null);
        assertThat(decision).isEqualTo(FORWARDED);
    }
    ...
    void dryRun_regexMatch_anotherValue() {
        ...
        RouteDecision decision = routerService.dryRun("{\"type\":\"NOTIFY_USER\"}", null, null);
        assertThat(decision).isEqualTo(FORWARDED);
    }
    ...
    void dryRun_noMatch_returnsNull() {
        RouteDecision decision = routerService.dryRun("{\"type\":\"UNKNOWN\"}", null, null);
        assertThat(decision).isNull();
    }

    @Test
    void dryRun_missingField_returnsNull() {
        RouteDecision decision = routerService.dryRun("{\"amount\":100}", null, null);
        assertThat(decision).isNull();
    }
    ...
    void dryRun_nonJson_returnsNull() {
        ...
        RouteDecision decision = routerService.dryRun("plain text", null, null);
        assertThat(decision).isNull();
    }

    // ------------------------------------------------------------------
    // route() — reactive (returns Mono)
    // ------------------------------------------------------------------
    ...
    void route_matchingMessage_returnsTrue() {
        ...
        assertThat(routerService.route(msg).block()).isEqualTo(FORWARDED);
    }
    ...
    void route_noMatchingRule_returnsFalse() {
        ...
        assertThat(routerService.route(msg).block()).isEqualTo(PASS);
    }
    ...
    void routerWithNoRules_routeReturnsFalse() {
        ...
        assertThat(emptyRouter.route(
                RacerMessage.create("ch", "{\"type\":\"ORDER\"}", "s")).block()).isEqualTo(PASS);
    }

    // ------------------------------------------------------------------
    // route() — error propagation and cycle prevention
    // ------------------------------------------------------------------

    @Test
    void route_matchingMessage_emitsForwarded() {
        RacerMessage msg = RacerMessage.create("racer:messages", "{\"type\":\"ORDER\"}", "svc");

        RouteDecision decision = routerService.route(msg).block();

        assertThat(decision).isEqualTo(FORWARDED);
    }

    @Test
    void route_noMatchingRule_emitsPass() {
    ...
    @Test
    void route_redisFailure_propagatesError() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> routerService.route(msg).block())
                .isInstanceOf(RuntimeException.class)
                .hasMessageContaining("Redis connection refused");
    }

    @Test
    void route_alreadyRouted_emitsPass() {
        RacerMessage msg = RacerMessage.create("racer:messages", "{\"type\":\"ORDER\"}", "svc");
    ...
    void functionalRouter_matchingMessage_forwardsViaPublishTo() {
        ...
        RouteDecision decision = svc.route(msg).block();
    ...
    void functionalRouter_publishToError_propagatesError() {
        ...
        RacerMessage msg = RacerMessage.create("ch", "{\"type\":\"ORDER\"}", "svc");
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> svc.route(msg).block())
                .isInstanceOf(RuntimeException.class)
