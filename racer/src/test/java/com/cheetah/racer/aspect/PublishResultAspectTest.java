...
class PublishResultAspectTest {
    ...
    void setUp() {
        ...
        when(annotation.priority()).thenReturn(PriorityLevel.NONE);
    ...
    void priority_mono_routesViaPriorityPublisher() throws Throwable {
        ...
        when(priorityPub.publish(anyString(), any(), anyString(), any(PriorityLevel.class)))
    ...
    void multiChannel_selectivePriority_onlyAnnotatedChannelGetsPriority() throws Throwable {
        ...
        when(annotation.priority()).thenReturn(PriorityLevel.HIGH);
        ...
        verify(priorityPub, times(1))
                .publish(eq(CHANNEL), eq("selective-payload"), eq("test-sender"), eq(PriorityLevel.HIGH));
        ...
        verify(priorityPub, never())
                .publish(eq(auditChannel), any(), anyString(), any(PriorityLevel.class));
    }
    ...
    void priority_concurrent_flux_routesViaPriorityPublisher() throws Throwable {
        ...
        verify(priorityPub, times(3)).publish(eq(CHANNEL), any(), eq("test-sender"), eq(PriorityLevel.HIGH));
        verify(publisher, never()).publishAsync(any(), anyString());
    }
    ...
    void resolvePriorityLevel_racerMessageWithPriority_usesMessagePriority() throws Throwable {
        ...
        RacerMessage msgWithPriority = RacerMessage.create(CHANNEL, "content", "svc", PriorityLevel.LOW);
