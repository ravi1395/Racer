...
class RacerPriorityPublisherTest {
    ...
    void publish_highPriority_sendsToCorrectSubChannel() {
        ...
        StepVerifier.create(publisher.publish("racer:orders", "payload", "svc", PriorityLevel.HIGH))
    ...
    void publish_lowPriority_sendsToCorrectSubChannel() {
        ...
        StepVerifier.create(publisher.publish("racer:orders", "payload", "svc", PriorityLevel.LOW))
                .assertNext(count -> assertThat(count).isEqualTo(1L))
                .verifyComplete();
    ...
    @Test
    void publish_normalPriority_sendsToNormalSubChannel() {
        when(schemaRegistry.validateForPublishReactive(anyString(), anyString())).thenReturn(Mono.empty());

        StepVerifier.create(publisher.publish("racer:orders", "payload", "svc", PriorityLevel.NORMAL))
                .assertNext(count -> assertThat(count).isEqualTo(1L))
                .verifyComplete();
    ...
    @Test
    void publish_highPriority_subChannelNameIsUpperCase() {
        when(schemaRegistry.validateForPublishReactive(anyString(), anyString())).thenReturn(Mono.empty());
        ...
        // sub-channel is always formed as <base>:priority:HIGH (upper-case enum name)
        verify(redisTemplate).convertAndSend(eq("racer:orders:priority:HIGH"), anyString());
    }
    ...
    void publish_withoutSchemaRegistry_works() {
        ...
        StepVerifier.create(pubNoSchema.publish("ch", "data", "svc", PriorityLevel.NORMAL))
                .assertNext(count -> assertThat(count).isEqualTo(1L))
                .verifyComplete();
    }
    ...
    void publish_error_propagates() {
        ...
        StepVerifier.create(publisher.publish("ch", "data", "svc", PriorityLevel.HIGH))
