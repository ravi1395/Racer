...
class PriorityLevelTest {
    ...
    @Test
    void none_hasMaxIntWeight() {
        assertThat(PriorityLevel.NONE.getWeight()).isEqualTo(Integer.MAX_VALUE);
    }

    @Test
    void none_weightExceedsAllOtherLevels() {
        assertThat(PriorityLevel.NONE.getWeight())
                .isGreaterThan(PriorityLevel.LOW.getWeight());
    }

    // ------------------------------------------------------------------
    // of() — happy paths (backward-compat, delegates to fromString)
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // fromString() — @JsonCreator, happy paths
    // ------------------------------------------------------------------

    @Test
    void fromString_high_caseInsensitive() {
        assertThat(PriorityLevel.fromString("HIGH")).isEqualTo(PriorityLevel.HIGH);
        assertThat(PriorityLevel.fromString("high")).isEqualTo(PriorityLevel.HIGH);
        assertThat(PriorityLevel.fromString("High")).isEqualTo(PriorityLevel.HIGH);
    }
    ...
    void fromString_normal() {
        assertThat(PriorityLevel.fromString("NORMAL")).isEqualTo(PriorityLevel.NORMAL);
        assertThat(PriorityLevel.fromString("normal")).isEqualTo(PriorityLevel.NORMAL);
    }

    @Test
    void fromString_low() {
        assertThat(PriorityLevel.fromString("LOW")).isEqualTo(PriorityLevel.LOW);
        assertThat(PriorityLevel.fromString("low")).isEqualTo(PriorityLevel.LOW);
    }

    @Test
    void fromString_withWhitespace_isTrimmed() {
        assertThat(PriorityLevel.fromString("  HIGH  ")).isEqualTo(PriorityLevel.HIGH);
        assertThat(PriorityLevel.fromString(" low ")).isEqualTo(PriorityLevel.LOW);
    }

    // ------------------------------------------------------------------
    // fromString() — fallback / NONE guard
    // ------------------------------------------------------------------

    @ParameterizedTest
    @NullSource
    @ValueSource(strings = {"", "   ", "UNKNOWN", "CRITICAL", "emergency"})
    void fromString_invalidOrMissing_returnsNormal(String input) {
        assertThat(PriorityLevel.fromString(input)).isEqualTo(PriorityLevel.NORMAL);
    }

    @Test
    void fromString_noneInput_returnsNormal() {
        // NONE must never surface from the wire — deserialising it yields NORMAL
        assertThat(PriorityLevel.fromString("NONE")).isEqualTo(PriorityLevel.NORMAL);
        assertThat(PriorityLevel.fromString("none")).isEqualTo(PriorityLevel.NORMAL);
    }
}
